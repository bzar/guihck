...
static const char GUIHCK_GUILE_DEFAULT_SCM[] =
    ...
    "(define (alias? x) (and (pair? x) (eq? 'alias (car x))))"

    "(define (create-element type props children)"
    "  (define (set-properties! props aliases)"
    "    (if (null? props) '()"
    "      (begin"
    "        (if (eq? (alias? (cadr props)) aliases)"
    "          (set-element-property! (car props) (cadr props))"
    "          #t)"
    "        (set-properties! (cddr props) aliases))))"
    "  (lambda ()"
    "    (begin"
    "      (push-new-element! type)"
    "      (set-properties! props #f)"
    "      (map (lambda (c) (c)) children)"
    "      (set-properties! props #t)"
